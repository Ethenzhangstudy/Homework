...
--columns ProductSubcategoryID and Color from the Production.Product table. We do not want any rows that are NULL.in any of the two columns in the result. 
SELECT DISTINCT *
FROM (SELECT ProductSubcategoryID
FROM Production.Product
WHERE ProductSubcategoryID IS NOT NULL) a CROSS JOIN (SELECT Color
FROM Production.Product
